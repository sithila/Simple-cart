...
if(isset($_POST["load_cart"]) && $_POST["load_cart"]==1)
{
	if(isset($_SESSION["products"]) && count($_SESSION["products"])>0){ //if we have session variable
		$cart_box = '<ul class="added-items cart-products-loaded">';
		$total = 0;
		foreach($_SESSION["products"] as $product){ //loop though items and prepare html content
			$cart_box .=  '<li>' . $product["name"]. ' (Qty : ' . $product["qty"]. ') &mdash; ' . $currency. sprintf("%01.2f", ($product["price"] * $product["qty"])). ' <a href="#cart" class="remove-item" data-code="'.$product["code"].'">&times;</a></li>';
			$subtotal = ($product["price"] * $product["qty"]);
			$total = ($total + $subtotal);
		}
		$cart_box .= "</ul>";
		$cart_box .= '<div class="cart-products-total">Total : '.$currency.sprintf("%01.2f",$total).' <u><a class="blue-link" href="#cart" title="Review Cart and Check-Out">Check-out</a></u></div>';
